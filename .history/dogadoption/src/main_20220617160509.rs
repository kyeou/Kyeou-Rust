...
struct propose_dispose {
    ...
    dog_tentative: Vec<i32>
    
}

fn read_input( pd:propose_dispose, vals: &Vec<String>) {

     for i in 0.._n {
        let vec = vals[i as usize].split(" ").collect::<Vec<&str>>();
         for j in 0..(_n*2) {
             pd.owners_desire[i as usize][j as usize] = vec[j as usize].parse::<i32>().unwrap();
         }
     }

  for i in pd._num_of_owners..(pd._num_of_owners + (pd_n * 2)) {
         let vec = vals[i as usize].split(" ").collect::<Vec<&str>>();
         for j in 0.._n {
             pd.dogs_desire[(i - _n) as usize][j as usize] = vec[j as usize].parse::<i32>().unwrap();
         }
     }
...
let mut _pd = propose_dispose {
    _num_of_owners: _n,
    owners_desire: vec![vec![0;( _n*2) as usize]; (_n) as usize],
    dogs_desire:  vec![vec![0; (_n) as usize]; (_n*2) as usize],
    owners_tenative: vec![vec![-1; (2) as usize]; (_n) as usize],
    dog_tentative: vec![-1; (_n*2) as usize]
};
