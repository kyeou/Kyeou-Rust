...
fn check_unstable(i: i32, j: i32, map: &Vec<Vec<i32>>, visited: &Vec<Vec<bool>> )  {
    
  if !(i < 0 || j < 0 || i >= len(map) || j >= len(map[0]) || map[i as usize][j as usize] != 0 || visited[i as usize][j as usize]){ visited[i][j] = true;
