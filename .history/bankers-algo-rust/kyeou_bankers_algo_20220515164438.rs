...
let mut _res_amt: i32;
 ...
 let mut _final_sequence : Vec<i32>;
    ...
    counter = 0;
    for i in &mut _proc_vec {
        print!("Enter number of units of each resource (r0 to r%d) Allocated to process p%d: ",counter, (_res_amt-1));
        std::io::stdout().flush().unwrap();
        read_line_input!(temp_vec, "");
        for j in &mut temp_vec {
        i.max_amt.push(*j);
...
fn main() {
    ...
    loop {
        println!("\nBatch Scheduling");
        println!("--------------------------------");
        println!("1) Enter parameters");
        println!("2) Run the Banker's algorithm to determine a safe sequence");
        println!("3) Quit program and free memory\n");
        read_input!(_choice, "Make a selection: ");
        match _choice {
            1=>{ent_parms()},
            2=>println!("2 chosen"),
            3=>{println!("Quitting"); break;},
