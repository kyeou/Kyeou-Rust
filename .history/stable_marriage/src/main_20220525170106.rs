...
fn check_unstable(curr_man_id: i32, curr_woman_id: i32, matchings: Vec<i32>, mens_desire: Vec<Vec<i32>>, womens_desire: Vec<Vec<i32>>) -> bool {
    ...
    for i in 0..matchings.len() {
        ...
        if  herr_curr_match == womens_desire[(curr_woman_id - 1) as usize][i] {
            pref_of_her_match = i;
        }
    }

    for i in 0..matchings.len()  {
        if curr_man_id ==  womens_desire[(curr_woman_id - 1) as usize][i] {
            pref_of_curr_man = i;
        }
    }

    for i in 0..matchings.len()  {
        if (curr_woman_id == mens_desire[(curr_man_id - 1) as usize][i as ]) {
            pref_of_currWoman = i;
        }
    }
