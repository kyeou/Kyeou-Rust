...
println!("{:#?}", vals);
let mut mensDesire = vec![vec![0; _n as usize]; _n as usize];
for i in 0.._n {
    let vec = vals[i as usiz].split(" ").collect::<Vec<&str>>();
    for j in 0.._n {
   mensDesire[i as usize][j as usize] = vec[i as usize].parse::<i32>().unwrap(); 
}
}
println!("{:#?}", mensDesire);
//println!("{}", _n);
