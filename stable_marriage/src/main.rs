...
fn check_unstable(
    ...
    curr_woman_id: i32,
    matchings: &Vec<i32>,
    mens_desire: &Vec<Vec<i32>>,
    womens_desire: &Vec<Vec<i32>>,
) -> bool {
    ...
    for i in 0..matchings.len() {
        if matchings[(curr_man_id - 1) as usize]
            == mens_desire[(curr_man_id - 1) as usize][i as usize]
        {
            pref_of_his_match = i as i32;
        }
    }
...
fn main() {
    ...
    println!(
        "{:?}",
        check_unstable(1, 3, &matchings, &mens_desire, &womens_desire)
    );

    for i in 1..(_n + 1) {
        for j in 1..(_n + 1) {
            if check_unstable(i as i32, j as i32, &matchings, &mens_desire, &womens_desire) {
                not_stable = not_stable + 1;
            }
        }
    }
    println!("Instabilities: {:?}", not_stable);
}
